...
The runner evaluates step `when:` guards (default True) and deep-resolves
payloads before dispatching to providers via the Registry.
"""
...
# type alias for clarity; mirrors Registry.Middleware
Middleware = Callable[
...
# --------------------------------------------------------------------------------------
# Middleware loading from environment
# --------------------------------------------------------------------------------------

def load_middlewares_from_env() -> List[Middleware]:
    """
    Load middlewares listed in SPINE_MIDDLEWARES env var.
    Format: "pkg.mod:fn,pkg.other:mw"
    """
    raw = (os.getenv("SPINE_MIDDLEWARES") or "").strip()
    if not raw:
        return []
...
class Spine:
    ...
    def dispatch_capability(
        ...
        *,
        ...
        task = Task(
            envelope=env,
            payload_schema=(payload_schema or capability),
    ...
    def load_pipeline_and_run(
        ...
        *,
        ...
        for idx, step in enumerate(data.get("steps") or [], 1):
            ...
            # Capture a baton-like object to expose as ${<step_id>.result}
    ...
    def _resolve(self, obj: Any, state: Dict[str, Any]) -> Any:
        """
        Deep-resolve ${...} in strings, lists, dicts. If a string is exactly a
        single ${...} token, the resolved value is returned *as-is* (can be a
        non-string like list/dict/bool). Otherwise tokens are stringified.
        """
    ...
    def _eval_expr(self, expr: str, state: Dict[str, Any]) -> Any:
        ...
        if c_idx != -1:
            ...
            # Treat None/empty-string as missing so defaults work even if an env var is set but blank
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        ok4 = len(arts4) == 1 and (arts4[0].meta or {}).get("result", {}).get("echo", {}).get("z") == 3
