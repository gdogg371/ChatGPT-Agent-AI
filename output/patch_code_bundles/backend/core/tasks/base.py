...
class TaskAdapter(Protocol):
    ...
    def sanitize(self, payload: Any, item: Dict[str, Any]) -> str:
        """
        Normalize/clean payload coming back from the LLM so it's safe and deterministic
        for application and verification. Returns a string payload suitable for apply().
        """
        ...

    def apply(self, original_src: str, item: Dict[str, Any], payload: str) -> str:
        """
        Apply the sanitized payload to the given source text, returning the updated text.
        Implementations must be pure (no I/O) and deterministic.
        """
        ...
    ...
    def parse_response(self, raw: str, expected_ids: List[str]) -> List[Dict[str, Any]]:
        """
        Convert a raw LLM response into a list of results: [{ "id": str, "payload": Any, ... }]
        """
