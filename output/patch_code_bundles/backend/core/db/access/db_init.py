...
# Lazy, one-time schema bootstrap flag
_SCHEMA_INIT_DONE = False

def _create_all_if_needed() -> None:
    """
    Lazily create tables for all known ORM bases.
    We import inside the function to avoid import cycles at module import time.
    """
    global _SCHEMA_INIT_DONE
    if _SCHEMA_INIT_DONE:
        return
    try:
        # Each model module defines its own DeclarativeBase `Base`
        from v2.models.introspection_index import Base as IntrospectBase  # type: ignore
        from v2.models.agent_insights import Base as AgentInsightsBase     # type: ignore

        IntrospectBase.metadata.create_all(_engine)
        AgentInsightsBase.metadata.create_all(_engine)

        _SCHEMA_INIT_DONE = True
        _log_info("[database] ✅ Schema ensured (introspection_index, agent_insights).")
    except Exception as e:
        _log_warn(f"[database] ⚠ Failed to ensure schema: {e}")

def get_sqlalchemy_session():
    """
    Returns a new SQLAlchemy session object.
    Ensures schema exists on first call.
