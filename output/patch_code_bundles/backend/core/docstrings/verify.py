...
def _payload(task_like: Any) -> Dict[str, Any]:
    if isinstance(task_like, dict): return task_like
...
def verify_batch_v1(task: Any, context: Dict[str, Any] | None = None) -> List[Artifact]:
    ...
    for it in items:
        ...
        if not (ds.startswith("\n") and ds.endswith("\n")):
            (errs if policy == "strict" else warns).append("docstring must start and end with a newline" if policy == "strict" else "docstring should start and end with a newline")
        ...
        if over:
            (errs if policy == "strict" else warns).append(f"{'lines exceed' if policy=='strict' else str(len(over))+' line(s) exceed'} width {width}")
