...
def _verify_batch_v1(payload: Dict[str, Any]) -> List[Artifact]:
    ...
    Input:
      - sanitized | items: List[{"id","docstring","signature", ...}]

    Output:
      meta.result = {
        "reports": [{"id","ok","issues":[..]}],
        "items":   [ ... filtered to only OK items ... ]
      }
    """
    ...
    def _is_trivial(s: str) -> bool:
        ...
        banned = {
            ...
            "todo", "tbd", "n/a", "na", "none", "no description", "summary",
        }
        if t in banned:
            return True
        words = [w for w in t.replace("\n", " ").split(" ") if w]
        return len(words) < 8  # very short → trivial
    ...
    for it in items:
        ...
        issues.extend(list(i1) + list(i2))

        if _is_trivial(doc):
            issues.append("Docstring is trivial/placeholder.")

        ok = (not issues)
        reports.append({"id": str(it.get("id")), "ok": ok, "issues": issues})
        if ok:
            ok_items.append(it)

    return _ok({"reports": reports, "items": ok_items}, uri="spine://docstrings/verify")
