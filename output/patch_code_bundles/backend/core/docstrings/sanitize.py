...
def sanitize_docstring(
    ...
    **_: object,  # tolerate extra keys from upstream without failing (e.g., 'route', 'schema', etc.)
) -> str:
    """
    ...
    ----------
    raw : str
        Raw text produced by the model (may contain triple quotes).
    signature : Optional[str]
        Python-ish signature; used to infer kind when symbol_kind is absent.
    symbol_kind : Optional[str]
        Explicit symbol kind: "module" | "class" | "function".
    **_ : object
        Ignored extra keyword args for forward compatibility.
    """
