...
def build_user_prompt(batch: List[Dict[str, Any]]) -> str:
    """
    Build the user prompt by embedding a slimmed view of items (id, mode,
    signature, description, contextual code). The model must return JSON
...
def _salvage_items_from_text(raw: str, expected_ids: List[str]) -> List[Dict[str, Any]]:
    """
    Last-resort recovery: scan for ANY balanced dicts and pick those that look
    like items. This lets us recover partial results when the outer JSON is truncated.
    """
...
class UnpackResultsStep:
    ...
    def run(self, raw: str) -> List[Dict[str, Any]]:
        ...
        Parse the model's response into a list of items:
          { "id": str, "mode": "create|rewrite", "docstring": str, "extras": {...} }
