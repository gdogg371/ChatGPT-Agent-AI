...
def build_user_prompt(batch: List[Dict[str, Any]]) -> str:
    """
    Build the user prompt by embedding a slimmed view of items (id, mode, signature,
    description, contextual code). The model must return JSON as instructed in the
    system prompt.
    """
    slim = [
        {
...
if __name__ == "__main__":
    ...
    # 3) Salvage from messy text (code fence + smart quotes + trailing comma)
    messy = """```json
    { “items”: [ {"id": "A", "docstring": "Doc A",}, {"id": "B", "mode": "rewrite", "docstring": "Doc B"} ] }
    ``` extra chatter...
    """
