...
This module replaces the previous monolithic Engine class that directly
imported DB, LLM, and patching code. It now serves as a **provider**
entrypoint for the Spine capability `llm.engine.run.v1`.
...
def _build_variables(payload: Dict[str, Any]) -> Dict[str, Any]:
    """
    Prepare a variables dict for the pipeline. We mirror common names to
    uppercase so YAML can refer to them as ${VAR}.
    """
