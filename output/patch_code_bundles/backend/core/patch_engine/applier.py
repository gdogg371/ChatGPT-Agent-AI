# File: backend/core/patch_engine/applier.py
...
def _parse_header_path(line: str, prefix: str) -> Optional[Path]:
    """
    Parse a '--- path' or '+++ path' line. Strips optional 'a/' or 'b/' prefixes
    and discards timestamps (after a tab). Returns None if path is '/dev/null'.
    """
...
class PatchApplier:
    ...
    def _dry_run(self, files: List[FilePatch]) -> Tuple[bool, dict[Path, List[str]]]:
        ...
        for fp in files:
            ...
            if rel_old is None and rel_new is not None:
                ...
                read_path = self.workspace / (rel_old or rel_new)  # rel_old preferred
                if not read_path.exists():
                    read_path = self.workspace / (rel_new or rel_old)
                if read_path.exists():
                    base_lines = self._read_lines(read_path)
                else:
                    base_lines = []
            ...
            if fp.old_path is None and fp.new_path is not None:
                ...
                if target_path.exists():
                    ...
                    if new_lines:
                        ...
                        self._write_lines(target_path, new_lines)
                    else:
                        target_path.unlink()
                continue
            else:
    ...
    def _try_apply_to_lines(self, old_lines: List[str], hunks: List[Hunk]) -> Tuple[bool, List[str]]:
        """
        Attempt to apply hunks to old_lines. Returns (ok, new_lines_if_ok).
        Exact matching only (no fuzz).
        """
        # Work on a copy
        new_lines: List[str] = []
