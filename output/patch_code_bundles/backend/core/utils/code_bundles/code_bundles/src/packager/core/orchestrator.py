...
import base64, json, hashlib, inspect
...
def _effective_ci(cfg: Any) -> bool:
    """
    Determine case-insensitive behavior:
      1) Use cfg.effective_case_insensitive() if present,
      2) else use bool(cfg.case_insensitive) if provided,
      3) else default True on Windows, False otherwise.
    """
    fn = getattr(cfg, "effective_case_insensitive", None)
    if callable(fn):
        try:
            return bool(fn())
        except Exception:
            pass
    val = getattr(cfg, "case_insensitive", None)
...
class SourceIngestor:
    ...
    def ingest(self, external_root: Path) -> List[Path]:
        ...
        eng = DiscoveryEngine(DiscoveryConfig(
            ...
            case_insensitive=_effective_ci(self.cfg),
...
class Packager:
    ...
    def run(self, external_source: Optional[Path] = None) -> PackagerResult:
        ...
        # --- Build run-spec with compatibility handling ---
        runspec = self._build_runspec_compat(prompts_public)
    ...
    # ---- run-spec compatibility builder -------------------------------------
    def _build_runspec_compat(self, prompts_public: Optional[dict]) -> dict:
        """
        Call RunSpecWriter.build_snapshot with whatever signature it exposes.
        If both known signatures fail, return a minimal runspec so the pipeline continues.
        """
        meta = {"source_root": str(self.cfg.source_root), "emitted_prefix": self.cfg.emitted_prefix}

        fn = getattr(RunSpecWriter, "build_snapshot", None)
        if callable(fn):
            try:
        ...
        if t:
            for k in ("split_bytes", "chunk_bytes", "parts_index_name",
                ...
                if hasattr(t, k):
                    transport[k] = getattr(t, k)

        fallback = {
            "version": "1",
            "bundle": self.cfg.out_bundle.name if hasattr(self.cfg, "out_bundle") else "design_manifest.jsonl",
            "provenance": dict(meta),
            "transport": transport,
            "prompts": prompts_public or None,
        }
