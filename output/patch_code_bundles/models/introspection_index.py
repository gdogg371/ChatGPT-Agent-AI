...
from sqlalchemy import (
    ...
    Index,
    UniqueConstraint,
    text,  # ✅ SQLite-safe server defaults
)

# NOTE:
# Use SQLite-friendly server defaults for timestamps. `func.now()` compiles to
# NOW() which SQLite doesn't implement. `CURRENT_TIMESTAMP` works across SQLite.
...
class IntrospectionIndex(Base):
    ...
    # ✅ Use SQLite-safe CURRENT_TIMESTAMP for server defaults
    ...
    updated_at: Mapped[datetime] = mapped_column(
        ...
        # server_onupdate is honored by many backends; on SQLite this still yields CURRENT_TIMESTAMP at update time
