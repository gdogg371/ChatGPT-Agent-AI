...
def _match_any(rel_posix: str, globs: List[str], case_insensitive: bool = False) -> bool:
    ...
    if case_insensitive:
        ...
        globs = [g.casefold() for g in globs]
    for g in globs:
        g = g.replace("\\", "/")
...
def _seg_excluded(parts: Tuple[str, ...], segment_excludes: List[str], case_insensitive: bool = False) -> bool:
    ...
    for seg in parts[:-1]:  # ignore filename itself
        if (seg.casefold() if case_insensitive else seg) in segs:
            return True
    return False
...
def discover_repo_paths(
    *,
    ...
    for cur, _dirs, files in os.walk(src_root, followlinks=follow_symlinks):
        # directory pruning by segment excludes
        # (we can't easily prune dirs here without mutating _dirs; simpler to skip per-file)
        for fn in sorted(files):
            ...
            # segment excludes
            if _seg_excluded(p.relative_to(src_root).parts, segment_excludes, case_insensitive):
                continue
...
def _publish_to_github(cfg: NS, code_items_repo_rel: List[Tuple[Path, str]]) -> None:
    ...
    - code_items_repo_rel: [(local_path, repo_relative_path)] from the repo,
      filtered by include/exclude/segment rules.
    - artifacts from cfg.* are pushed under 'design_manifest/' at repo root.
    """
    ...
    if cfg.publish.publish_codebase:
        print(f"[packager] Publish(GitHub): code ({len(code_items_repo_rel)} files)")
...
def main() -> int:
    ...
    # GITHUB: publish code (repo-relative) + artifacts under design_manifest/
    if mode in {"github", "both"}:
        # Optional remote clean before publish
