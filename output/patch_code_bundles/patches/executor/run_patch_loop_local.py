...
Key points
----------
- Logic mirrors `run_patch_loop.py`; this variant just picks convenient defaults.
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Run the LLM patch loop (local).")
    ...
    try:
        ...
        engine = Engine(cfg)
        run_dir = engine.run()
        print(f"[run_patch_loop_local] Completed. Artifacts at: {run_dir}")
        return 0
