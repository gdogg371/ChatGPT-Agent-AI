...
def load_publish_config() -> PublishOptions:
    ...
    candidates = [
        ...
        SECRETS_DIR / "publish.local.json",        # ← your actual path
        _user_config_dir() / "publish.local.json", # ← OS fallback
    ]

    print("[run_pack] publish.local.json candidates:")
    for c in candidates:
        print(" ", "-", str(c), "EXISTS" if c.exists() else "missing")
    ...
    opts = PublishOptions(
        ...
        publish_transport=bool(cfg_data.get("publish_transport", True)),
        publish_prompts=bool(cfg_data.get("publish_prompts", True)),
        clean_before_publish=bool(cfg_data.get("clean_before_publish", False)),
    )
    ...
    print("[run_pack] publish flags:",
          ...
          "clean_before_publish=", getattr(opts, "clean_before_publish", False))
...
EXTERNAL_SOURCE = Path(r"C:\\Users\\cg371\\PycharmProjects\\ChatGPT Bot\\v2\\backend\\core\\utils\\code_bundles\\code_bundles\\")
