...
This provider **normalizes row shape** so downstream capabilities (enrichment,
prompt build, sanitize, patch) receive the canonical keys:

    id, filepath, lineno, name, symbol_type, description, status, unique_key_hash?

Why this matters
----------------
`retriever.enrich.v1` (SourceRetriever) expects `filepath` + `lineno`.
If we return `file`/`line` instead, enrichment finds 0 items even though the
database has rows.
...
Payload (optional):
  ...
  - status_any: list[str]
  - max_rows: int
  - exclude_globs: list[str]        # repo-relative POSIX globs
  - segment_excludes: list[str]     # case-insensitive directory basenames
"""
...
def _problem(uri: str, code: str, message: str, *,
...
def _row_to_item(row: Mapping[str, Any]) -> Dict[str, Any]:
    """
    Normalize a DB row dict to the canonical engine shape expected by enrichment:

        id, filepath, lineno, name, symbol_type, description, status, unique_key_hash?

    Accepts legacy/alternate column names: file→filepath, line→lineno, filetype→symbol_type, hash→unique_key_hash.
    """
    # Column name compatibility
    filepath = row.get("filepath") or row.get("file") or row.get("relpath") or row.get("path") or ""
    filepath = _as_posix(filepath)

    symbol_type = row.get("symbol_type") or row.get("filetype") or row.get("type") or "unknown"
    ...
    uid = (row.get("id")
           or row.get("unique_key_hash")
           or row.get("hash")
           or f"{filepath}:{lineno}")
    ...
    return {
        ...
        "status": status,
        # optional but useful for downstream correlation
...
def fetch_v1(task: Task, context: Dict[str, Any]) -> List[Artifact]:
    """
    Query introspection_index and return **canonical** target items (see module docstring).
    """
    uri_ok = "spine://result/introspect.fetch.v1"
    uri_ng = "spine://problem/introspect.fetch.v1"

    p = task.payload or {}
    url = str(p.get("sqlalchemy_url") or p.get("url") or "").strip()
    table = str(p.get("sqlalchemy_table") or p.get("table") or "").strip()
    ...
    max_rows = int(p.get("max_rows") or p.get("limit") or 200)
    ...
    # Fetch without WHERE to avoid dialect/string normalization pitfalls; filter in Python for reliability.
    ...
    for r in rows:
        ...
        if not item["filepath"]:
            continue
        if status_any and _norm_status(item.get("status", "")) not in status_any:
            continue
        if _path_excluded(item["filepath"], exclude_globs=exclude_globs, segment_excludes=segment_excludes):
            continue
