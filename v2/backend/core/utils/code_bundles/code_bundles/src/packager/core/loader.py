...
def _family_canon_map(aliases: Mapping[str, str], metadata_emission: Mapping[str, str]) -> Dict[str, str]:
    """
    Build a canonicalization map using ONLY provided aliases from YAML.
    - includes provided aliases as-is
    - ensures every canonical family maps to itself
    - adds normalized ('.' -> '_') synonyms for both alias keys and canonical values
    """
    def _norm(s: str) -> str:
        return s.replace(".", "_")

    # Start with YAML-provided aliases (authoritative)
    canon: Dict[str, str] = {str(k): str(v) for k, v in (aliases or {}).items()}

    # Ensure every canonical family maps to itself (both dotted and normalized spellings)
    for fam in (metadata_emission or {}).keys():
        fam_s = str(fam)
        canon.setdefault(fam_s, fam_s)
        canon.setdefault(_norm(fam_s), _norm(fam_s))

    # Add normalized synonyms for all known alias pairs
    # e.g., if YAML has "ast.call" -> "ast_calls", also add "ast_call" -> "ast_calls"
    for k, v in list(canon.items()):
        nk, nv = _norm(k), _norm(v)
        canon.setdefault(nk, nv)
...
def load_packager_config(repo_root: Path) -> Config:
    ...
    Uses manifest_paths to remove hardcoded filenames/locations.
    """
    ...
    # ── Use manifest_paths (authoritative) to eliminate hardcoded paths
    mp = raw.get("manifest_paths", {}) or {}
    root_dir = _req_str(mp, "root_dir")
    analysis_subdir = _req_str(mp, "analysis_subdir")
    parts_index_filename = _req_str(mp, "parts_index_filename")

    def _resolve_under_root(p: str) -> Path:
        pth = Path(p)
        return pth if pth.is_absolute() else (repo_root / pth)

    design_manifest_dir = _resolve_under_root(root_dir).resolve()
    parts_index_path = (design_manifest_dir / parts_index_filename).resolve()
    analysis_out_dir = (design_manifest_dir / analysis_subdir).resolve()
